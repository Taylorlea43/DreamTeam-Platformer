...
public class DiscoPlayer extends Player
{
	...
	{
		 super(new SpriteSheet(ImageLoader.load("discoPlayer.png"), 45, 85), x, y, "STAND_RIGHT", false, level);
	        gravity = .5f;
		...
		{
			{
				...
				put("DISCO_RIGHT", new Frame[] {
						new FrameBuilder(spriteSheet.getSprite(0, 1), 200).withScale(1).withBounds(1, 1, 40, 80).build(),
						new FrameBuilder(spriteSheet.getSprite(0, 1), 200).withScale(1).withBounds(1, 1, 40, 80).build()
						});

				put("DISCO_LEFT",
						new Frame[] {
								new FrameBuilder(spriteSheet.getSprite(0, 1), 200).withScale(1)
										.withImageEffect(ImageEffect.FLIP_HORIZONTAL).withBounds(1, 1, 40, 80).build(),
								new FrameBuilder(spriteSheet.getSprite(0, 1), 200).withScale(1)
										.withImageEffect(ImageEffect.FLIP_HORIZONTAL).withBounds(1, 1, 40, 80).build()
						});
