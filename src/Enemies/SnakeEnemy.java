...
public class SnakeEnemy extends Enemy {
	...
	public SnakeEnemy(Point location, Direction facingDirection) {
	        super(location.x, location.y, new SpriteSheet(ImageLoader.load("Snake Enemy.png"), 25 , 18), "WALK_LEFT");
		...
		return new HashMap<String, Frame[]>() {
			{
				put("WALK_RIGHT", new Frame[] {
						new FrameBuilder(spriteSheet.getSprite(0, 0), 100).withScale(2)
								.withImageEffect(ImageEffect.FLIP_HORIZONTAL).withBounds(3, 4, 23, 13).build(),
						new FrameBuilder(spriteSheet.getSprite(0, 1), 100).withScale(2)
								.withImageEffect(ImageEffect.FLIP_HORIZONTAL).withBounds(3, 4, 23, 13).build() });

				put("WALK_LEFT",
						new Frame[] {
								new FrameBuilder(spriteSheet.getSprite(0, 0), 100).withScale(2).withBounds(3, 4, 23, 13)
										.build(),
								new FrameBuilder(spriteSheet.getSprite(0, 1), 100).withScale(2).withBounds(3, 4, 23, 13)
										.build() });
			}
		};
	}
