...
public class BonusTileset extends Tileset {

    public BonusTileset() {
        super(ImageLoader.load("BonusTileset.png"), 24, 24, 3);
    }
    ...
    public ArrayList<MapTileBuilder> defineTiles() {
        ...
        //blue ground
        Frame blueDiscoFrame = new FrameBuilder(getSubImage(3, 3))
                .withScale(tileScale)
                .build();

        MapTileBuilder blueDiscoTile = new MapTileBuilder(blueDiscoFrame)
                .withTileType(TileType.NOT_PASSABLE);

        mapTiles.add(blueDiscoTile);

        //pink ground
        Frame pinkDiscoFrame = new FrameBuilder(getSubImage(3, 4))
                .withScale(tileScale)
                .build();

        MapTileBuilder pinkDiscoTile = new MapTileBuilder(pinkDiscoFrame)
                .withTileType(TileType.NOT_PASSABLE);

        mapTiles.add(pinkDiscoTile);

        //disco sky
        Frame discoSkyFrame = new FrameBuilder(getSubImage(3, 5))
                .withScale(tileScale)
                .build();

        MapTileBuilder discoSkyTile = new MapTileBuilder(discoSkyFrame);

        mapTiles.add(discoSkyTile);

        // black disco sky
        Frame blackDiscoSkyFrame = new FrameBuilder(getSubImage(4, 0))
                .withScale(tileScale)
                .build();

        MapTileBuilder blackDiscoSkyTile = new MapTileBuilder(blackDiscoSkyFrame);

        mapTiles.add(blackDiscoSkyTile);
