...
public class WolfEnemy extends Enemy {
	...
	public WolfEnemy(Point location, Direction facingDirection) {
	        super(location.x, location.y, new SpriteSheet(ImageLoader.load("WolfEnemy.png"), 48 , 29), "WALK_LEFT");
		...
		return new HashMap<String, Frame[]>() {
			{
				put("WALK_RIGHT", new Frame[] {
						new FrameBuilder(spriteSheet.getSprite(0, 0), 100).withScale(2)
								.withImageEffect(ImageEffect.FLIP_HORIZONTAL).withBounds(1, 1, 40, 28).build(),
						new FrameBuilder(spriteSheet.getSprite(0, 1), 100).withScale(2)
								.withImageEffect(ImageEffect.FLIP_HORIZONTAL).withBounds(1, 1, 40, 28).build() });

				put("WALK_LEFT",
						new Frame[] {
								new FrameBuilder(spriteSheet.getSprite(0, 0), 100).withScale(2).withBounds(1, 1, 40, 28)
										.build(),
								new FrameBuilder(spriteSheet.getSprite(0, 1), 100).withScale(2).withBounds(1, 1, 40, 28)
										.build() });
			}
		};
	}
